...
//    $smarty->assign('sortedProducts', sortedProductsEndpoint())
...
function adminRemoveCategoryEndpoint()
{
    ...
    if (strlen($categoryId) < 32) {
        header("Location: /?action=adminCategories&message=Wrong ID given");
    }
    $categories = getCategoriesList();
    ...
    for ($i = 0; $i < $len; $i++) {
        ...
        if (!$deleted) {
            header("Location: /?action=adminCategories&message=404. The category does not exist");

        }
    }
    writeFile(CATEGORIES_FILE, array_values($categories));
    header("Location: /?action=adminCategories&message=The category with id=$id has been removed!");
...
function adminRemoveUserEndpoint()
{
    ...
    for ($i = 0; $i < $len; $i++) {
        ...
        if (!$deleted) {
            header("Location: /?action=adminUsers&message=The user does not exist.");
        }
    }
    writeFile(USERS_FILE, array_values($users));
    header("Location: /?action=adminUsers&message=The user has been terminated.");
}
...
function adminRemoveProductEndpoint()
{
    ...
    for ($i = 0; $i < $len; $i++) {
        ...
        if (!$deleted) {
            header("Location: /?action=adminProducts&message=The product does not exist.");
        }
    }
    writeFile(PRODUCTS_FILE, array_values($products));
    header("Location: /?action=adminProducts&message=The product has been removed");
}
...
function adminChangeRoleEndpoint()
{
    ...
    if ($admin === null) {
        header("Location: /?action=adminUsers&error= You did not specify the user role");
        return;
    }
    if ($id === 0) {
    ...
    if (!$updated) {
        header("Location: /?action=adminUsers&error= User was not found");
        return;
    }

    writeFile(USERS_FILE, $users);

    header("Location: /?action=adminUsers&message= You updated role for user with id $id");
    return;
}
...
function adminAddProductEndpoint()
{
    ...
    header("Location: /?action=adminProducts&message= The product has been added successfully");
}
...
//    }
    ...
    $sortedProducts = array_filter($products, function ($product) {
        $categoryId = $_GET['categoryId'];
        if ($categoryId === $product['category_id']) {
            return $product;
        }
    });
    if ($_GET['action'] === "sorted") {
